...
function ImageUploader({ value, onChange, label }: { value: string; onChange: (url: string) => void; label: string }) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!res.ok) { setErr(data?.error ?? `Gagal mendapat URL upload (${res.status}).`); return; }
    ...
    } catch (e: any) { setErr(e?.message ? `Upload gagal: ${e.message}` : "Upload gagal. Periksa koneksi."); }
